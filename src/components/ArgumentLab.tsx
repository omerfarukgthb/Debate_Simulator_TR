    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id="argument-lab-section">
      ...
      <div className="lg:col-span-5 space-y-4">
        <div className={`${cardClass} rounded-xl p-5 border transition-all duration-300`}>
          ...
          <p className={`${textMutedClass} text-sm leading-relaxed mb-4`}>
            Bir münazara tezi seç, kendi tarafını belirle yapıp ilk fikirlerini yaz. Jürimiz seni tarafsızca puanlarken, bilge koçun kelimelerini nasıl şampiyon bir argümana dönüştürebileceğini gösterecek!
          </p>

          <div className="space-y-4">
            {/* Topic Switcher */}
            <div>
              <label htmlFor="topic-select" className={`block text-sm font-bold ${labelClass} mb-2 uppercase tracking-wider`}>Münazara Konusu</label>
              
              <div className="flex gap-2 mb-2">
                <button
                  id="tab-predefined-topics"
                  onClick={() => setIsCustomTopic(false)}
                  className={`flex-1 py-1.5 px-3 rounded-lg text-sm font-bold border transition-all cursor-pointer ${
                    !isCustomTopic 
              ...
              {!isCustomTopic ? (
                <select
                  ...
                  className="w-full text-sm p-2.5 rounded-lg border border-slate-200 bg-slate-50 text-slate-700 font-medium focus:ring-1 focus:ring-amber-500 cursor-pointer"
                >
            ...
            {/* Position Selection */}
            <div>
              <span className={`block text-sm font-bold ${labelClass} mb-2 uppercase tracking-wider`}>Sen hangi tarafı savunuyorsun?</span>
              <div className="grid grid-cols-2 gap-3">
                <button
                  id="btn-pos-pro"
                  onClick={() => setPosition("pro")}
                  className={`py-2 px-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 border transition-all cursor-pointer ${
                    position === "pro"
                      ? "border-emerald-600 bg-emerald-50 text-emerald-800 ring-1 ring-emerald-500 font-extrabold scale-[1.01]"
                      : "border-slate-200 text-slate-600 bg-white hover:bg-slate-50"
                  }`}
                >
                  <ThumbsUp className={`w-4 h-4 shrink-0 col-span-2 ${position === "pro" ? "text-emerald-600" : "text-slate-400"}`} />
                  <div className="text-left">
                    <span className="block font-extrabold font-sans text-sm">Tez (Evet)</span>
                ...
                <button
                  ...
                  className={`py-2 px-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 border transition-all cursor-pointer ${
                    position === "con"
                      ? "border-red-600 bg-red-50 text-red-800 ring-1 ring-red-500 font-extrabold scale-[1.01]"
                      : "border-slate-200 text-slate-600 bg-white hover:bg-slate-50"
                  }`}
                >
                  <ThumbsDown className={`w-4 h-4 shrink-0 ${position === "con" ? "text-red-600" : "text-slate-400"}`} />
                  <div className="text-left">
                    <span className="block font-extrabold font-sans text-sm">Antitez (Hayır)</span>
                    <span className="text-xs font-normal text-slate-400 block mt-0.5">Karşıyım</span>
                  </div>
                </button>
              </div>
            </div>
            ...
            {!isCustomTopic && currentPredefinedTopic && (
              <div className="bg-slate-50 p-3.5 rounded-lg border border-slate-200">
                <div className="flex gap-2 items-start text-sm">
                  ...
                  <div>
                    <strong className="text-slate-800 font-bold block mb-1">Kullanabileceğin Yardımcı Fikirler:</strong>
                    <ul className="list-disc pl-4 mt-1 space-y-1.5 text-slate-650 text-xs">
                      {(position === "pro" ? currentPredefinedTopic.proArguments : currentPredefinedTopic.conArguments).map((arg, idx) => (
                        <li key={idx} className="italic leading-relaxed">{arg}</li>
                      ))}
                    </ul>
                  </div>
                </div>
            ...
            {/* Main Argument Input */}
            <div>
              <label htmlFor="argument-textarea" className={`block text-sm font-bold ${labelClass} mb-1.5 uppercase tracking-wider`}>Senin Argümanın</label>
              <textarea
                ...
                className={`w-full text-sm p-3 rounded-lg border ${inputClass} placeholder-slate-400 focus:outline-none`}
              />
            </div>
            ...
            <button
              ...
              disabled={loading}
              className={`w-full ${btnPrimary} py-3.5 rounded-xl text-sm font-bold transition-all shadow-sm flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.01]`}
            >
              {loading ? (
                <>
        ...
        {history.length > 0 && (
          <div className={`${cardClass} rounded-xl p-5 border shadow-sm`}>
            <h3 className={`text-sm font-extrabold ${labelClass} uppercase tracking-widest mb-3 flex items-center gap-1.5`}>
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
              {history.map((item) => (
                <div
                  ...
                  className="p-3 rounded-lg border border-slate-150 hover:border-amber-400 bg-slate-50/50 hover:bg-white text-left transition-all text-xs cursor-pointer flex justify-between items-start gap-1.5 text-slate-800"
                >
                  <div className="space-y-0.5 truncate flex-1">
                    <strong className="text-slate-800 block truncate text-xs font-bold">{item.topic}</strong>
                    <span className="text-[11px] text-slate-500 italic block truncate">"{item.argument}"</span>
                    ...
                    <span className="text-[10px] text-slate-450 ml-2">{item.timestamp}</span>
                  </div>
                  <button
      ...
      <div className="lg:col-span-7 space-y-4">
        {loading && (
          <div className={`${cardClass} rounded-xl p-12 border shadow-sm h-full flex flex-col justify-center items-center text-center`}>
            ...
            <p className={`${textMutedClass} text-sm max-w-sm mt-2 leading-relaxed`}>
        ...
        {!loading && !evaluation && (
          <div className={`${cardClass} rounded-xl p-12 border shadow-sm h-full flex flex-col justify-center items-center text-center`}>
            ...
            <p className={`${textMutedClass} text-sm max-w-xs mt-1 leading-relaxed`}>
        ...
        {evaluation && (
          ...
          >
            {/* 1. Scorecard Panel */}
            <div className={`${cardClass} rounded-xl p-5 border shadow-sm`}>
              <span className={`font-mono text-xs ${accentColor} font-bold tracking-widest uppercase mb-1.5 block`}>Jüri Değerlendirme Raporu</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div id="score-structure" className="bg-slate-50 p-3 rounded-xl border border-slate-150 text-center text-slate-800">
                  <span className="text-xs text-slate-600 font-bold block mb-1">Argüman Yapısı</span>
                ...
                <div id="score-evidence" className="bg-slate-50 p-3 rounded-xl border border-slate-150 text-center">
                  <span className="text-xs text-slate-600 font-bold block mb-1">Kanıt ve Destek</span>
                ...
                <div id="score-persuasion" className="bg-slate-50 p-3 rounded-xl border border-slate-150 text-center">
                  <span className="text-xs text-slate-600 font-bold block mb-1">İkna Gücü</span>
                ...
                <div id="score-fallacies" className="bg-slate-50 p-3 rounded-xl border border-slate-150 text-center">
                  <span className="text-xs text-slate-600 font-bold block mb-1" title="Tuzak hatalardan korunma derecesi">Tutarlılık</span>
                  <div className="text-2xl font-display font-extrabold text-rose-600">
            ...
            {evaluation.fallaciesFound && evaluation.fallaciesFound.length > 0 ? (
              <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex gap-3">
                ...
                <div>
                  <h4 className="font-display font-bold text-sm text-red-800 uppercase tracking-wider mb-1">Mantık Hatası (Safsata) Tuzağı Tespit Edildi! 🙅‍♂️</h4>
                  <ul className="list-disc pl-4 text-xs text-slate-700 space-y-1.5 mt-1 leading-relaxed">
                    {evaluation.fallaciesFound.map((fallacy, idx) => (
                      <li key={idx} className="font-semibold">{fallacy}</li>
                    ))}
                  </ul>
                  <span className="text-xs text-slate-500 font-medium block mt-2">İpucu: Sağ üstteki "Safsata Rehberi" sekmesine giderek bu tuzakların örneklerini inceleyebilirsin.</span>
                </div>
              </div>
            ) : (
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 flex gap-3 items-center">
                ...
                <div>
                  <h4 className="font-display font-bold text-sm text-emerald-800 uppercase tracking-wider">Kusursuz Mantıksal Tutarlılık! 🛡️</h4>
                  <p className="text-xs text-emerald-700 font-semibold mt-1 leading-relaxed">Argümanında mantık hatası/safsata tuzağına rastlanmadı. Jüri, safsata yapmama disiplininden ötürü seni tebrik ediyor!</p>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-emerald-50/20 rounded-xl p-4 border border-emerald-100/80">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <h4 className="font-display font-extrabold text-sm text-emerald-800 uppercase tracking-wider">Öğretmenimin Övgüleri</h4>
                </div>
                <ul className="space-y-20 flex flex-col gap-2">
                  {evaluation.positives.map((pos, idx) => (
                    <li key={idx} className="text-sm text-slate-650 flex gap-2 items-start leading-relaxed font-medium">
              ...
              <div className="bg-red-50/10 rounded-xl p-4 border border-red-100/50">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <h4 className="font-display font-extrabold text-sm text-amber-800 uppercase tracking-wider">Jüriden Yapıcı Eleştiriler</h4>
                </div>
                <ul className="space-y-20 flex flex-col gap-2">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="bg-amber-600 px-4 py-3 flex items-center gap-2">
                ...
                <h4 className="font-display font-bold text-sm text-white uppercase tracking-wider">Şampiyon Münazara Örneği ("Şöyle Yazabilirdin")</h4>
              </div>
              <div className="p-4 bg-amber-50/10 border-b border-dashed border-slate-100">
                <div className="flex items-start gap-2">
                  ...
                  <p className="font-serif font-semibold text-slate-800 text-sm leading-relaxed whitespace-pre-line select-all">
                    "{evaluation.improvedExample}"
                  </p>
                </div>
              </div>
              <div className="p-3 bg-slate-50 text-xs text-slate-500 font-semibold flex items-center justify-between">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 shadow-inner">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-3">
                  <div>
                    <strong className="text-slate-800 text-sm font-bold font-mono">Bilge Koç Köşesi:</strong>
                    <p className="text-slate-650 font-medium text-sm mt-1.5 leading-relaxed">
                      {evaluation.overallSummary}
                    </p>
                  </div>

                  <div className="pt-2 border-t border-slate-200">
                    <strong className="text-slate-700 text-xs font-bold uppercase tracking-wider mb-1 block">Bir Sonraki Adım İçin Taktikler:</strong>
                    <ul className="list-disc pl-4 mt-1.5 text-xs text-slate-650 space-y-1.5">
